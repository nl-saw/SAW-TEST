...
@$uri = filter_var(strip_tags($_GET['uri']), FILTER_SANITIZE_URL);
@$title = strip_tags($_GET['title']);
$charsValid = array(' ', '-', '!', '?');
if(!ctype_alnum(str_replace($charsValid, '', $title))) {
	die();
}
if (!empty($_GET['title'])) { $title = '<h3 class="window-title">'.$title.'</h3>'; } else { $title=""; }
if (!empty($_GET['width'])) { $width = "width:".(int)$_GET['width']."px"; } else { $width=""; }
?>
